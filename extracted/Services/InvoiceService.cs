...
public class InvoiceService
{
    ...
    public Invoice Create(Invoice invoice, string deviceId, User? actor = null, IEnumerable<SalePayment>? payments = null)
    {
        ...
        foreach (var item in invoice.Items)
        {
            ...
            if (item.Quantity <= 0 || product.Quantity < item.Quantity) throw new InvalidOperationException($"Insufficient stock for {product.Name}. Available: {product.Quantity}");
        }
        db.Invoices.Add(invoice); db.SaveChanges();
        foreach (var item in invoice.Items)
        {
            item.InvoiceGuid = invoice.Guid; db.InvoiceItems.Add(item);
