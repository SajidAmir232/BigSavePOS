...
public class ProductService
{
    ...
    public Product Save(Product product, User? actor = null)
    {
        ...
        product.UpdatedAtUtc = DateTime.UtcNow; product.IsSynced = false; product.Sku = string.IsNullOrWhiteSpace(product.Sku) ? GenerateSku(db) : product.Sku.Trim(); product.Barcode = string.IsNullOrWhiteSpace(product.Barcode) ? GenerateBarcode(db, product.BranchId) : product.Barcode.Trim().Trim('\'', '"');
